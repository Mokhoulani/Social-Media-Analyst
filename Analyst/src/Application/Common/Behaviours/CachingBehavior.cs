    ...
    where TRequest : IQuery<TResponse>
{
    ...
    {
        if (typeof(TRequest).GetCustomAttributes(typeof(CacheAttribute), false).FirstOrDefault() is not CacheAttribute
            cacheAttribute)
        {
            return await next();
        }
        ...
        return await cache.GetOrSetAsync(cacheKey, async (token) => await next(),
            options => options.SetDuration(TimeSpan.FromSeconds(cacheAttribute.DurationInSeconds))
                .SetFailSafe(true)
                .SetFactoryTimeouts(TimeSpan.FromSeconds(5)), token: cancellationToken);
    }
